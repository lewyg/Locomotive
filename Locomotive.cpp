...
static vector<glm::vec3> parts_position = {
	...
	glm::vec3(0.0f, 0.5f, 0.5f),	//cylinder
	glm::vec3(0.0f, 0.5f, 2.0f),	//cone
...
Locomotive::Locomotive(GLfloat size)
{
	...
	this->parts.push_back(new Box(1.0f * size, 1.5f * size, 1.0 * size, tex1, scalePosition(parts_position[0], size), start_rotation[0]));
	this->parts.push_back(new Cylinder(1.5f * size, 0.5f * size, 20, tex1, scalePosition(parts_position[1], size), start_rotation[2]));
	this->parts.push_back(new Cone(0.5f * size, 0.5f * size, 20, tex1, scalePosition(parts_position[2], size), start_rotation[2]));
	this->parts.push_back(new Box(1.0f * size, 0.3f * size, 2.5f * size, tex1, scalePosition(parts_position[3], size), start_rotation[0]));

	//wheels
	PrimitiveObject * wheel = new Cylinder(0.2f * size, 0.3f * size, 20, tex2, scalePosition(parts_position[4], size), start_rotation[4]);
	this->parts.push_back(wheel);
	wheel->setRotation(start_rotation[1]);
	this->wheels.push_back(wheel);
	wheel = new Cylinder(0.2f * size, 0.3f * size, 20, tex2, scalePosition(parts_position[5], size), start_rotation[4]);
	this->parts.push_back(wheel);
	wheel->setRotation(start_rotation[1]);
	this->wheels.push_back(wheel);
	wheel = new Cylinder(0.2f * size, 0.3f * size, 20, tex2, scalePosition(parts_position[6], size), start_rotation[5]);
	this->parts.push_back(wheel);
	wheel->setRotation(start_rotation[1]);
	this->wheels.push_back(wheel);
	wheel = new Cylinder(0.2f * size, 0.3f * size, 20, tex2, scalePosition(parts_position[7], size), start_rotation[5]);
	this->parts.push_back(wheel);
	wheel->setRotation(start_rotation[1]);
	this->wheels.push_back(wheel);

	//side_rods
	PrimitiveObject * rod = new Box(0.05f * size, 0.1f * size, 1.7f * size, tex1, scalePosition(parts_position[8], size), start_rotation[0]);
	this->parts.push_back(rod);
	this->side_rods.push_back(rod);
	rod = new Box(0.05f * size, 0.1f * size, 1.7f * size, tex1, scalePosition(parts_position[9], size), start_rotation[0]);
	this->parts.push_back(rod);
	this->side_rods.push_back(rod);
}
